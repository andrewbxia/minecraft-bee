...
// branding visuals
const axia = eid("branding"), axiatxt = axia.innerText;
axia.innerText = "";
...
function playbsplash() {
    if(!axia.matches(":hover")){
        const t = bsplash.pass();
        setTimeout(() => {
            playbsplash(randint(1), randint(1, 1));
        }, t + randint(400, 100));
    }
    else{
...
const barsizealert = () => {
    if(trueheight / window.innerHeight < 0.75){
        alert(`
            hi! thanks for checking my site :)\n
